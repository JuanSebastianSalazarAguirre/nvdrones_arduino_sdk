#include "I2C.h"

I2C::I2C(){}

I2C::I2C(SerialIO *_serialIO, Callback *_callback) {
  serialIO = _serialIO;
  callbacks = _callback;
}

void I2C::deviceID(uint8_t ID){
  serialIO->sendPacket(ID, 11, 7);
}
void I2C::beginTransmission(void){
  serialIO->sendPacket((int16_t)0, 11, 8);
}

void I2C::endTransmission(void){
  serialIO->sendPacket((int16_t)0, 11, 4);
}

void I2C::write(uint8_t data){
  serialIO->sendPacket((uint8_t)data, 11, 3);
}

void I2C::read(void){
  serialIO->sendPacket((int16_t)0, 11, 5);
}
void I2C::wireRequest(uint8_t bytes){
  serialIO->sendPacket(bytes, 11, 6);
}

void I2C::readCallback(void (*cb)(byte)) {
  callbacks->i2cRead = cb;
}
